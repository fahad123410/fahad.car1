...
# Use SQLite in /tmp on Vercel, and an absolute instance/premium_rental.db locally.
...
class Config:
    ...
    FIREBASE_ENABLED = bool(
        ...
        or (FIREBASE_SERVICE_ACCOUNT_PATH and os.path.exists(FIREBASE_SERVICE_ACCOUNT_PATH))
...
def _get_firestore_client():
    ...
    if not app.config.get("FIREBASE_ENABLED"):
        logger.info("Firebase is disabled in config")
        return None
    ...
    try:
        if not firebase_admin._apps:
            ...
            if app.config.get("FIREBASE_STORAGE_BUCKET"):
                options["storageBucket"] = app.config["FIREBASE_STORAGE_BUCKET"]
...
def _firestore_upsert(collection: str, doc_id: str, payload: dict):
    ...
    try:
        ...
        logger.debug(f"Firestore upsert successful: {collection}/{doc_id}")
    except Exception as e:
        logger.error(f"Firestore upsert failed for {collection}/{doc_id}: {e}")
...
def _restore_from_firestore_if_empty():
    ...
    """
    if not app.config.get("FIREBASE_ENABLED"):
        logger.info("Firebase not enabled, skipping restore")
        return
        ...
        try:
            ...
            for doc in docs:
                ...
                try:
                    ...
                    logger.exception(
                        f"Failed to deserialize Firestore doc {doc.id} in {coll_name}: {e}"
                    )
...
# Register Firestore sync if enabled
if app.config.get("FIREBASE_ENABLED"):
                    ...
                    <div style="background: rgba({{ hero.overlay_color }}, {{ hero.overlay_opacity }}); padding: 60px 30px; border-radius: 20px;">
                        <div class="text-center text-white">
                            {% if active_banner %}
                                ...
                                {% else %}
                                <!-- Default Special Offer Banner (Enhanced styling) -->
                                <div class="offer-banner mb-4"
                                     style="
                                         display: inline-block;
                                         background: linear-gradient(135deg, #ff9a1f 0%, #ff512f 35%, #ff0066 100%);
                                         color: #000;
                                         padding: 18px 26px;
                                         border-radius: 18px;
                                    ...
                                    <div class="d-flex flex-column flex-md-row align-items-center justify-content-center gap-2">
                                        <div class="text-start text-md-center mb-2 mb-md-0">
                                            <div style="font-size: 15px; font-weight: 700; text-transform: uppercase;">
                                                🔥 Limited Time Offer
                                            </div>
                                            <div style="font-size: 13px; color: rgba(0,0,0,0.7);">
                                                Ideal for weddings, events & airport drops
                                            </div>
                                        </div>
                                        <div class="d-flex align-items-baseline ms-md-3">
                                            <div style="font-size: 22px; font-weight: 700; margin-right: 10px;">
                                                300 KM
                                            </div>
                ...
                <div class="col-12">
                    <div class="glass-card">
                        <div class="card-body p-3">
                            ...
                            <a href="{{ url_for('admin_simple_dashboard') }}" class="btn-gradient me-2" style="background: linear-gradient(135deg, #6c757d 0%, #495057 100%);">
                                <i class="fas fa-chart-pie"></i> Simple Money View
                            </a>
                            <a href="{{ url_for('admin_dashboard') }}" class="btn-outline-gradient">
...
def admin_debug_firebase():
    ...
    if firestore_client:
        return f"FIREBASE_ENABLED={app.config.get('FIREBASE_ENABLED')}<br>Firestore client initialized successfully!"
    else:
        return f"FIREBASE_ENABLED={app.config.get('FIREBASE_ENABLED')}<br>Firestore client not available. Check logs for details."
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="{{ url_for('admin_simple_dashboard') }}"><i class="fas fa-chart-pie"></i> Money View</a>
                    </li>
                    <li class="nav-item">
...
if __name__ == '__main__':
    ...
    print("21. FIREBASE SYNC (if enabled): All changes mirrored to Firestore")
    print("22. FIREBASE RESTORE: On Vercel, data auto-restored from Firestore")
    print("23. SIMPLE MONEY DASHBOARD: Daily income + pending customers")
    print("="*70)
